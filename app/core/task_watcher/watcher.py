...
class TaskWatcher:
    ...
    def add_watch(self, folder: str, pattern: str = "*.task.md") -> bool:
        ...
        try:
            ...
            if not os.path.exists(folder):
                logger.error(f"[TaskWatcher] 文件夹不存在: {folder}")
                return False
            
            if folder in self._watch_folders:
                logger.debug(f"[TaskWatcher] 文件夹已在监听中: {folder}")
                return True
            ...
            self._watch_folders[folder] = pattern
            
            logger.info(f"[TaskWatcher] 开始监听文件夹: {folder}, pattern={pattern}")
            return True
        except Exception as e:
            logger.error(f"[TaskWatcher] 添加文件夹监听失败: {e}")
            return False
    ...
    def remove_watch(self, folder: str) -> bool:
        ...
        try:
            ...
            if folder in self._handlers:
                del self._handlers[folder]
            
            if folder in self._watch_folders:
                del self._watch_folders[folder]
            
            # 如果没有监听项了，停止观察者
            if not self._watch_folders and self._observer:
                self._observer.stop()
                self._observer = None
            
            logger.info(f"[TaskWatcher] 移除文件夹监听: {folder}")
            return True
        except Exception as e:
            logger.error(f"[TaskWatcher] 移除文件夹监听失败: {e}")
            return False
    ...
    def _on_file_detected(self, file_path: str, config: TaskConfig) -> None:
        ...
        if self._callback:
            try:
                ...
                logger.error(f"[TaskWatcher] 执行回调失败: {e}")
        else:
            logger.debug(f"[TaskWatcher] 无回调配置，任务待处理: {file_path}")
    ...
    def load_watches_from_config(self, config_store: Any) -> int:
        ...
        configs = config_store.get_by_trigger_mode("file_change")
